...
std::pair<std::error_code, asio::ip::tcp::socket>
connect(asio::io_context &io_context, std::string host, std::string port) {
...
public:
  ...
  virtual async_simple::IOExecutor *getIOExecutor() override { return nullptr; }
...
template <typename Socket, typename AsioBuffer> struct ReadSomeAwaiter {
public:
...
template <typename Socket, typename AsioBuffer> struct ReadAwaiter {
public:
...
template <typename Socket, typename AsioBuffer>
inline async_simple::coro::Lazy<std::pair<std::error_code, size_t>>
async_read(Socket &socket, AsioBuffer &buffer) noexcept {
  co_return co_await ReadAwaiter{socket, buffer};
}

template <typename Socket, typename AsioBuffer> struct ReadUntilAwaiter {
public:
...
template <typename Socket, typename AsioBuffer> struct WriteAwaiter {
public:
...
template <typename Socket, typename AsioBuffer>
inline async_simple::coro::Lazy<std::pair<std::error_code, size_t>>
async_write(Socket &socket, AsioBuffer &&buffer) noexcept {
...
async_connect(asio::io_context &io_context, asio::ip::tcp::socket &socket,
