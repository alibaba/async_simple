...
public:
    ...
    // 4-bits priority_level, less level is more important. Default
    // value of async-simple schedule is DEFAULT_LEVEL. For scheduling level >=
    // 8, executor shouldn't always execute the work immediately if other works
    // are waiting.
    enum priority_level {
        HIGHEST_LEVEL = 0,
        DEFAULT_LEVEL = 7,
    ...
    // Low 16-bit of schedule_hint is reserved for async-simple, and the lowest
    ...
    // shouldn't always execute the work immediately when other works are
    // waiting.
    virtual bool schedule(Func func, [[maybe_unused]] uint64_t schedule_hint) {
    ...
    bool schedule_move_only(util::move_only_function<void()> func,
                            [[maybe_unused]] uint64_t schedule_hint) {
        MoveWrapper<decltype(func)> tmp(std::move(func));
        return schedule([func = tmp]() { func.get()(); }, schedule_hint);
    }
    ...
    TimeAwaitable after(Duration dur, uint64_t schedule_hint = DEFAULT_LEVEL);
...
protected:
    ...
    virtual void schedule(Func func, Duration dur, uint64_t schedule_hint) {
...
public:
    TimeAwaiter(Executor *ex, Executor::Duration dur, uint64_t schedule_hint)
        : _ex(ex), _dur(dur), _schedule_hint(schedule_hint) {}
...
public:
    ...
    void await_suspend(coro::CoroHandle<PromiseType> continuation) {
        _ex->schedule(std::move(continuation), _dur, _schedule_hint);
    }
...
private:
    ...
    uint64_t _schedule_hint;
};

// Awaitable to implement Executor::after.
class Executor::TimeAwaitable {
public:
    TimeAwaitable(Executor *ex, Executor::Duration dur, uint64_t schedule_hint)
        : _ex(ex), _dur(dur), _schedule_hint(schedule_hint) {}

    auto coAwait(Executor *) {
        return Executor::TimeAwaiter(_ex, _dur, _schedule_hint);
    }
...
Executor::TimeAwaitable inline Executor::after(Executor::Duration dur,
    ...
    return Executor::TimeAwaitable(this, dur, schedule_hint);
}
