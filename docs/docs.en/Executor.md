...
public:
    ...
    virtual bool schedule(Func func, uint64_t schedule_hint);
    virtual bool schedule(Func func, Duration d);
    virtual bool schedule(Func func, Duration d, uint64_t schedule_hint);
    bool schedule_move_only(util::move_only_function<void()> func);
    bool schedule_move_only(util::move_only_function<void()> func, uint64_t schedule_hint);
...
- `schedule_hint` is used to guide the scheduling. Low 16 bits are reserved for async-simple, and lowest 4 bits is stand for priority level of schedule. Larger number indicating lower priority. Default priority is `async_simple::Executor::DEFAULT_LEVEL`. The scheduling logic isn't necessary, which is determined by implementation. However, to avoid spinlock deadlock, when schedule_hint is equal or greater than `async_simple::Executor::YIELD_LEVEL`, scheduler shouldn't always execute the work immediately when other works are waiting.
